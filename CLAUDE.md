...
- 14 issues filed across 3 milestones. **Milestone 1** (Prototype: MCP talking to an LLM) has #1/#2/#6 done, #3/#4/#5 open. **Milestone 2** (Full FatSecret feature coverage) is **fully closed** — #6 through #10 all done. **Milestone 3** (Harden & containerize) is open: #11 (token persistence), #12 (Dockerfile), #13 (more test coverage), #14 (more docs) remain.
...
- **#13/#14** — more test coverage (tool handlers, OAuth2 once it's verified) and more docs (Docker section once #12 lands); both already partially done.
