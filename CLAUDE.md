...
- **Working end to end today, against the real API and this developer's real FatSecret account**: `FatSecretMcp.slnx` + `src/FatSecretMcp/` (multi-targets `net8.0;net10.0`), `ModelContextProtocol.AspNetCore` 2.2.0, stateless Streamable HTTP at `/mcp`. Registered with Claude Code as `fatsecret-local-dev` (`http://localhost:5102/mcp`) — used live in-session for real tasks (reviewing this year's weight trend, backfilling missing breakfast entries, logging dinner, computing an August calorie-intake-vs-burned report from live diary + exercise data).
...
- **Target framework**: multi-targets `net8.0;net10.0` — net8.0 is the still-widely-installed LTS (supported to ~Nov 2026), net10.0 is the current LTS (to ~Nov 2028); a single-target net10.0-only tool would exclude anyone who hasn't upgraded their runtime yet, and net8.0-only would be an EOL trap within months of publishing. No RID matrix, strong-name signing, or crypto-specific constraints needed for this project (unlike Foundation).
